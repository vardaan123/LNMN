...
def train(epoch_id, model, architect, criterion, optimizer, train_set, val_set_architect, batch_size, device, summary_writer, n_epochs, lr, unrolled, reg_coeff):
    ...
    for batch_id, (image, question, q_len, answer, _) in enumerate(pbar):
        ...
        # torch.cuda.empty_cache()
        # get_gpu_memory_map()
        architect.step((image, question, q_len), answer, (image_search, question_search, q_len_search), answer_search, lr, optimizer, unrolled=unrolled)
        # print('flag 2')
        # get_gpu_memory_map()
        ...
        # torch.cuda.empty_cache()
        # get_gpu_memory_map()
        output, imp_loss = model(image, question, q_len)
        # torch.cuda.synchronize()
        # torch.cuda.empty_cache()
        # get_gpu_memory_map()
