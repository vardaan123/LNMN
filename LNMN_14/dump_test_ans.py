...
from dataset_test import ClevrDataset

from model import Stack_NMN
from collections import Counter
import utils_test
from utils_test import collate_data
...
def test(model, test_set, batch_size, device, ansix_tow):
    ...
    f1 = open('test_pred.txt','w')

    with torch.no_grad():
        for batch_id, (image, question, q_len) in enumerate(pbar):
            image, question = (
                image.to(device),
                question.to(device),
            )
...
if __name__=='__main__':
    ...
    dictionaries = utils_test.build_dictionaries(args.clevr_dir)
    ...
    model = Stack_NMN(args.max_stack_len, args.max_time_stamps, args.n_modules, n_choices, args.n_nodes, n_words, args.embed_size, args.lstm_hid_dim, args.input_feat_dim, args.map_dim, args.mlp_hid_dim, args.mem_dim, args.kb_dim, args.kernel_size, device).to(device)
    ...
    test_set = DataLoader(clevr_test, batch_size=args.batch_size, num_workers=1)
