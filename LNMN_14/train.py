...
def train(epoch_id, model, architect, criterion, optimizer, train_set, val_set_architect, batch_size, device, summary_writer, n_epochs, lr, unrolled, reg_coeff):
    ...
    for batch_id, (image, question, q_len, answer, _) in enumerate(pbar):
        ...
        # torch.cuda.empty_cache()
        # get_gpu_memory_map()
        output, imp_loss, _ = model(image, question, q_len, epoch_id)
        # torch.cuda.synchronize()
        # torch.cuda.empty_cache()
        # get_gpu_memory_map()
...
def valid(epoch_id, model, criterion, optimizer, val_set, batch_size, device, summary_writer, n_epochs):
    ...
    with torch.no_grad():
        for batch_id, (image, question, q_len, answer, family) in enumerate(pbar):
            ...
            output, _, _ = model(image, question, q_len, epoch_id)
...
def main():
    ...
    model = Stack_NMN(args.max_stack_len, args.max_time_stamps, args.n_modules, n_choices, args.n_nodes, args.temperature, n_words, args.embed_size, args.lstm_hid_dim, args.input_feat_dim, args.map_dim, args.mlp_hid_dim, args.mem_dim, args.kb_dim, args.kernel_size, device).to(device)
    ...
    optimizer = optim.Adam(filter(lambda p:p.requires_grad, model.module.network_parameters()), lr=args.lr)
