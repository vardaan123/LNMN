...
class Stack_NMN(nn.Module):
    def __init__(self, max_stack_len, max_time_stamps, n_modules, n_choices, n_nodes, temperature, vocab_size, embed_hidden, lstm_hid_dim, img_feat_dim, map_dim, mlp_hid_dim, mem_dim, kb_dim, kernel_size, device):
        ...
        self.module_list = nn.ModuleList(NetworkCell_3input(self.n_operations, self.temperature, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim) for _ in range(5))
        self.module_list.extend(nn.ModuleList(NetworkCell_4input(self.n_operations, self.temperature, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim) for _ in range(4)))
        self.module_list.extend(nn.ModuleList(NetworkAnswerCell_3input(self.n_operations, self.temperature, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.mem_dim) for _ in range(2)))
        self.module_list.extend(nn.ModuleList(NetworkAnswerCell_4input(self.n_operations, self.temperature, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.mem_dim) for _ in range(2)))
    ...
    def l2_l1_loss(self):
        ...
        for m in range(len(self.module_list)-1):
            for node in self.module_list[m].node_list:
                # print(node.op_weights)
                # print(F.softmax(node.op_weights, dim=0))
                # print(F.softmax(node.op_weights, dim=0).norm(2))
                loss += F.softmax(node.op_weights, dim=0).norm(2)
    ...
    def new(self):
        model_new = Stack_NMN(self.max_stack_len, self.max_time_stamps, self.n_modules, self.n_choices, self.n_nodes, self.temperature, self.vocab_size, self.embed_hidden, self.lstm_hid_dim, self.img_feat_dim, self.map_dim, self.mlp_hid_dim, self.mem_dim, self.kb_dim, self.kernel_size, self.device).to(self.device)
    ...
    # @profile
    def forward(self, image, ques, q_len, epoch_id):
        '''
        image: [64, 512, 15, 10]
        ques
        '''
        ...
        for t in range(self.max_time_stamps):
            ...
            for module_id, module in enumerate(self.module_list):
                ...
                if module_id < 5:
                    ...
                    '''
                    out = module(img_feat_mapped, a, text_feat_mapped, epoch_id) # [batch, 1, H, W]
                ...
                elif module_id >= 5 and module_id <= 8:
                    ...
                    out = module(img_feat_mapped, a1, a2, text_feat_mapped, epoch_id) # [batch, 1, H, W]
                    out = out.permute(0, 2, 3, 1) # [batch, H, W, 1]
                    ...
                    out = module(img_feat_mapped, text_feat_mapped, a, mem_prev, epoch_id) # [batch, n_choices]
                    # move ptr fw
                    '''
                    tmp_p = torch.zeros(batch_size, self.max_stack_len).to(self.device)
                    tmp_p.index_fill_(1, torch.tensor([self.max_stack_len-1]).to(self.device), 1)
                    ...
                    out = module(img_feat_mapped, text_feat_mapped, a1, a2, mem_prev, epoch_id) # [batch, n_choices]
        ...
        op_loss = self.l2_l1_loss()
        # print('op_loss = {}'.format(op_loss))

        return ans_logits, imp_loss, op_loss
