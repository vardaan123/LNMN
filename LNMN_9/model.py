...
class Stack_NMN(nn.Module):
    def __init__(self, max_stack_len, max_time_stamps, n_modules, n_choices, n_nodes, vocab_size, embed_hidden, lstm_hid_dim, img_feat_dim, map_dim, mlp_hid_dim, mem_dim, kb_dim, kernel_size, device, use_argmax):
        ...
        self.module_list = nn.ModuleList(NetworkCell_3input(self.n_operations, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.use_argmax) for _ in range(4))
        self.module_list.extend(nn.ModuleList(NetworkCell_4input(self.n_operations, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.use_argmax) for _ in range(2)))
        self.module_list.extend(nn.ModuleList(NetworkAnswerCell_3input(self.n_operations, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.mem_dim, self.use_argmax) for _ in range(1)))
        self.module_list.extend(nn.ModuleList(NetworkAnswerCell_4input(self.n_operations, self.kb_dim, 2*self.lstm_hid_dim, self.map_dim, self.mem_dim, self.use_argmax) for _ in range(1)))
