...
# import torch.distributions.dirichlet.Dirichlet as Dirichlet
# import torch.distributions.multinomial.Multinomial as Multinomial
...
class Node(nn.Module):
    def __init__(self, n_operations, use_argmax):
        super(Node, self).__init__()
        self.n_operations = n_operations # {min, max, +, .}
        self.op_weights = nn.Parameter(torch.randn(self.n_operations))
        self.choose_a = choose_a()
        self.choose_b = choose_b()
        self.use_argmax = use_argmax
        self.func_list = [lambda x1,x2:torch.min(x1, x2), lambda x1,x2:torch.max(x1, x2), lambda x1,x2:torch.add(x1, x2), lambda x1,x2:torch.mul(x1, x2), lambda x1,x2:self.choose_a(x1, x2), lambda x1,x2:self.choose_b(x1, x2)]

    def forward(self, x1, x2, epoch_id):
        # print(self.op_weights.data)
        # TODO: Use softmax instead of simple sum
        # print('op_weights requires_grad = {}'.format(self.op_weights.requires_grad))
        # out_dirichlet = torch.distributions.dirichlet.Dirichlet(self.op_weights.data).sample()
        # out_multinomial = torch.distributions.one_hot_categorical.OneHotCategorical(probs=out_dirichlet).sample()
        # temp = (1.0 - 0.9 * epoch_id * 1.0 / 20)
        # print('temp = {}'.format(temp))

        out_multinomial = F.softmax(self.op_weights, dim=0)
        # print('out_multinomial = {}'.format(out_multinomial))
        
        op_weights_norm = out_multinomial
        ...
        if not self.use_argmax:
            ...
            idx = op_weights_norm.argmax().item()
            # print('op_weights_norm = {}, idx= {}'.format(op_weights_norm, idx))
            out = op_weights_norm[idx] * self.func_list[idx](x1, x2).expand(max(x1.size(0), x2.size(0)), max(x1.size(1), x2.size(1)), max(x1.size(2), x2.size(2)), max(x1.size(3), x2.size(3)))

        # out = self.func_list[torch.nonzero(out_multinomial).item()](x1, x2).expand(max(x1.size(0), x2.size(0)), max(x1.size(1), x2.size(1)), max(x1.size(2), x2.size(2)), max(x1.size(3), x2.size(3)))
...
class NetworkCell_4input(nn.Module):
    def __init__(self, n_operations, img_feat_dim, text_feat_dim, map_dim, use_argmax):
        super(NetworkCell_4input, self).__init__()
        self.n_operations = n_operations
        self.use_argmax = use_argmax
        self.n_nodes = 3
        self.node_list = nn.ModuleList([Node(self.n_operations, self.use_argmax) for _ in range(self.n_nodes)])
    ...
    # @profile
    def forward(self, img_feat, attn_1, attn_2, c_txt, epoch_id):
        # self.train()
        '''
        Input
        ...
        attn = self.node_list[0](attn_1, attn_2, epoch_id) # [batch, map_dim, H, W]

        out_node_1 = self.node_list[1](img_feat, attn, epoch_id) # [batch, map_dim, H, W]
        # torch.cuda.synchronize()

        out_node_2 = self.node_list[2](out_node_1, c_txt_mapped, epoch_id) # [batch, map_dim, H, W]
        # torch.cuda.synchronize()
...
class NetworkCell_3input(nn.Module):
    ...
    # @profile
    def forward(self, img_feat, attn, c_txt, epoch_id):
        # self.train()
        '''
        Input
        img_feat : [batch, img_feat_dim, H, W]
        ...
        out_node_0 = self.node_list[0](img_feat, attn, epoch_id) # [batch, map_dim, H, W]
        # torch.cuda.synchronize()

        out_node_1 = self.node_list[1](out_node_0, c_txt_mapped, epoch_id) # [batch, map_dim, H, W]
        # torch.cuda.synchronize()
...
class NetworkAnswerCell_4input(nn.Module):
    def __init__(self, n_operations, img_feat_dim, text_feat_dim, map_dim, mem_dim, use_argmax):
    ...
    # @profile
    def forward(self, img_feat, c_txt, attn_1, attn_2, mem_in, epoch_id):
        # self.train()
        '''
        Input
        img_feat : [batch, img_feat_dim, H, W]
...
class NetworkAnswerCell_3input(nn.Module):
    ...
    # @profile
    def forward(self, img_feat, c_txt, attn, mem_in, epoch_id):
        # self.train()
        '''
        Input
        img_feat : [batch, img_feat_dim, H, W]
        ...
        # print('attn= {}'.format(attn.size()))
        out_node_0 = self.node_list[0](img_feat, attn, epoch_id) # [batch, map_dim, H, W]
        # print('out_node_0 = {}'.format(out_node_0.size()))
        out_node_1 = self.node_list[1](out_node_0, c_txt_mapped, epoch_id) # [batch, map_dim, H, W]
